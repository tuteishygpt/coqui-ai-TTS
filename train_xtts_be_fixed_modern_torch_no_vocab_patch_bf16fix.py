...
# Metadata normalized to formatter="ljspeech":
#   <stem>|<raw_text>|<normalized_text>
TRAIN_META_NORMALIZED = DATASET_DIR / "metadata_train.ljspeech.csv"
EVAL_META_NORMALIZED = DATASET_DIR / "metadata_eval.ljspeech.csv"
...
LANGUAGE = "be"
BATCH_SIZE = int(os.environ.get("XTTS_BATCH_SIZE", "2"))
GRAD_ACCUM_STEPS = int(os.environ.get("XTTS_GRAD_ACCUM_STEPS", "20"))
NUM_EPOCHS = int(os.environ.get("XTTS_NUM_EPOCHS", "1"))
LEARNING_RATE = float(os.environ.get("XTTS_LR", "9e-6"))
SAVE_STEP = int(os.environ.get("XTTS_SAVE_STEP", "540"))
START_WITH_EVAL = os.environ.get("XTTS_START_WITH_EVAL", "0") == "1"
OPTIMIZER_WD_ONLY_ON_WEIGHTS = True

# Modern torch speed knobs.
USE_AMP = os.environ.get("XTTS_USE_AMP", "1") == "1"
USE_TORCH_COMPILE = os.environ.get("XTTS_USE_TORCH_COMPILE", "0") == "1"  # experimental
TORCH_COMPILE_MODE = os.environ.get("XTTS_TORCH_COMPILE_MODE", "default")
TORCH_COMPILE_BACKEND = os.environ.get("XTTS_TORCH_COMPILE_BACKEND", "inductor")
ENABLE_TF32 = os.environ.get("XTTS_ENABLE_TF32", "1") == "1"
ENABLE_CUDNN_BENCHMARK = os.environ.get("XTTS_CUDNN_BENCHMARK", "1") == "1"
ENABLE_SDP_KERNELS = os.environ.get("XTTS_ENABLE_SDP_KERNELS", "1") == "1"
NUM_LOADER_WORKERS = int(os.environ.get("XTTS_NUM_LOADER_WORKERS", "2"))
PREFETCH_FACTOR = int(os.environ.get("XTTS_PREFETCH_FACTOR", "2"))
...
def preferred_amp_precision() -> str:
    env_override = os.environ.get("XTTS_AMP_PRECISION")
    if env_override:
...
AMP_PRECISION = preferred_amp_precision()
...
def enable_tf32_if_supported() -> bool:
    if not ENABLE_TF32:
        print("TF32 disabled by XTTS_ENABLE_TF32=0")
        return False
    ...
    # Newer PyTorch API (2.9+) when present.
    try:
    ...
    # Backward-compatible flags; many trainer summaries inspect these values.
    try:
...
def enable_runtime_speedups() -> None:
    ...
    print(f"XTTS_AMP_PRECISION raw env: {os.environ.get("XTTS_AMP_PRECISION")}")
    print(f"Torch compile requested: {USE_TORCH_COMPILE}")


# Enable runtime knobs as early as possible so trainer environment logging can see them.
...
def parse_reference_wav(meta_path: Path, dataset_dir: Path) -> str:
    for cols in iter_metadata_rows(meta_path):
        wav_path = resolve_dataset_wav(cols[0], dataset_dir)
        return str(wav_path)
    raise RuntimeError(f"No usable rows in metadata: {meta_path}")



def normalize_metadata_for_ljspeech(src_meta: Path, dst_meta: Path, dataset_dir: Path) -> Path:
    normalized_lines: list[str] = []
    checked = 0

    for cols in iter_metadata_rows(src_meta):
        if len(cols) < 2:
            continue
        ...
        normalized_lines.append(f"{wav_path.stem}|{text}|{text}")
        checked += 1

    if not normalized_lines:
        raise RuntimeError(f"No valid training rows after normalization: {src_meta}")

    dst_meta.write_text("\n".join(normalized_lines) + "\n", encoding="utf-8")
    print(f"Normalized metadata: {src_meta.name} -> {dst_meta.name} ({checked} rows)")
    return dst_meta
...
def apply_speed_config(config, trainer_args) -> None:
    # These are best-effort setters because trainer/config field names vary by version.
...
def maybe_compile_model(model):
    if not USE_TORCH_COMPILE:
        print("torch.compile disabled. Set XTTS_USE_TORCH_COMPILE=1 to enable it.")
        return model
    ...
    # Safer than compiling the whole trainer wrapper in many TTS stacks.
...
def main():
    ...
    train_meta_normalized = normalize_metadata_for_ljspeech(TRAIN_META, TRAIN_META_NORMALIZED, DATASET_DIR)
    eval_meta_normalized = normalize_metadata_for_ljspeech(EVAL_META, EVAL_META_NORMALIZED, DATASET_DIR)
    ...
    config_dataset = BaseDatasetConfig(
        ...
        meta_file_train=train_meta_normalized.name,
        meta_file_val=eval_meta_normalized.name,
        language=LANGUAGE,
    )
